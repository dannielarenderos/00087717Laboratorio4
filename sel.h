...
void createLocalC(Matrix &C,mesh m){
    ...
    float y = m.getParameter(VAR_Y);

    C.at(0).at(0) += -y/(3);    C.at(0).at(1) += y/(3);

    C.at(1).at(0) += -y/(3);    C.at(1).at(1) += y/(3);
...
void createLocalF(Matrix &F,mesh m){
   ...
   float j = m.getParameter(VAR_J);

    F.at(0).at(0) += -j/2;  F.at(0).at(1) += j/2;

    F.at(1).at(0) += -j/2;  F.at(1).at(1) += j/2;
...
Vector createLocalb(int element,mesh &m){
    ...
    float f = m.getParameter(VAR_W), l = m.getParameter(ELEMENT_LENGTH);
